...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payrolls = await prisma.payrollBatch.findMany({
      where,
      include: {
        _count: {
          select: {
    ...
    const transformed = await Promise.all(payrolls.map(async batch => {
      // Get totals from individual payrolls
      const payrollsForBatch = await prisma.payroll.findMany({
        where: { batchId: batch.id }
      })

      const totalGross = payrollsForBatch.reduce((sum, p) => 
        sum + parseFloat(p.grossTotal.toString()), 0
      )
      const totalNet = payrollsForBatch.reduce((sum, p) => 
        sum + parseFloat(p.netTotal.toString()), 0
      )
      const totalEmployer = payrollsForBatch.reduce((sum, p) => 
        sum + parseFloat(p.employerContributionTotal.toString()), 0
      )
      ...
      return {
        ...
        totalNet,
        totalEmployer,
        employeeCount: batch._count.payrolls,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calculate work days (simple: assume 22 days per month)
    const workDaysInMonth = 22
    const calendarDaysInMonth = periodEnd.getDate()
      ...
      data: {
        ...
        payrolls: {
          create: employees.map(emp => {
            // Simple calculation - should be enhanced with real payroll logic
            const grossSalary = parseFloat(emp.salary?.toString() || '0')
            const taxRate = 0.20 // 20% simplified
            const socialSecurityRate = 0.20 // 20% simplified
            ...
            const employerSocialSecurity = grossSalary * 0.20 // 20% AG-Anteil
            ...
            return {
              ...
              grossTotal: grossSalary,
              incomeTaxTotal: taxes,
              socialSecurityTotal: socialSecurity,
              netTotal: netSalary,
              employerContributionTotal: employerSocialSecurity
            }
          })
        }
      },

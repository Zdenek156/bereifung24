...
PGPASSWORD='***REMOVED***' pg_dump -U "$DB_USER" -h localhost "$DB_NAME" | gzip > "$SAFETY_BACKUP"
echo "Sicherheitsbackup: $SAFETY_BACKUP"

# Restore durchführen
echo "Restore wird durchgeführt..."
gunzip -c "$BACKUP_FILE" | PGPASSWORD='***REMOVED***' psql -U "$DB_USER" -h localhost "$DB_NAME" > /dev/null 2>&1

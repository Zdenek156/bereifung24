...
// Returns legal text with HTML content from the legal_texts table
export async function GET(
  ...
  try {
    ...
    // Support target parameter: 'app' (default for Flutter) or 'web'
    const target = request.nextUrl.searchParams.get('target') || 'app'

    // Use raw SQL since Prisma client may not have LegalText model generated
    const results: any[] = await prisma.$queryRawUnsafe(
      `SELECT key, title, content, version, target, updated_at as "updatedAt" FROM legal_texts WHERE key = $1 AND target = $2 LIMIT 1`,
      key, target
    )

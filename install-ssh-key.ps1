...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        ssh -i ~\.ssh\bereifung24_hetzner root@167.235.24.110 "cd /var/www/bereifung24 && sed -i '/^EMAIL_/d' .env && echo 'EMAIL_HOST=mail.your-server.de' >> .env && echo 'EMAIL_PORT=587' >> .env && echo 'EMAIL_SECURE=false' >> .env && echo 'EMAIL_USER=info@bereifung24.de' >> .env && echo 'EMAIL_PASSWORD=***REMOVED***' >> .env && echo 'EMAIL_FROM=info@bereifung24.de' >> .env && echo '' && echo '=== E-Mail Konfiguration ===' && grep '^EMAIL_' .env && echo '' && pm2 restart bereifung24"

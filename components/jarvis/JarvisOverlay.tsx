...
export default function JarvisOverlay() {
  ...
  const toggleMic = async () => {
    ...
    if (!SpeechRecognition) {
      setMicError('Spracheingabe wird von diesem Browser nicht unterstützt (nur Chrome/Edge).')
      return
    }

    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
      setMicError('Spracheingabe benötigt eine HTTPS-Verbindung.')
      return
    }
...
export default function JarvisOverlay() {
  ...
  const toggleMic = async () => {
    ...
    if (!SpeechRecognition) {
      setMicError('Spracheingabe wird von diesem Browser nicht unterstÃ¼tzt (nur Chrome/Edge).')
      return
    }

    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
      setMicError('Spracheingabe benÃ¶tigt eine HTTPS-Verbindung.')
      return
    }
    ...
    // Pre-request microphone permission for clearer error handling
    try {
      if (navigator.mediaDevices?.getUserMedia) {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
        stream.getTracks().forEach(t => t.stop())
    ...
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-gray-950/95 backdrop-blur-sm">
      ...
      <div className="w-full max-w-xl px-6 flex flex-col gap-2 mb-4 min-h-[120px]">
        {visibleMessages.map((msg, i) => (
          ...
          >
            ...
            >
              {msg.role === 'jarvis' && (
                <span className="text-cyan-500/70 mr-1">â€º</span>
              )}
              {msg.text}
            </p>
          </div>
        ...
        {(isThinking || isSpeaking) && (
          <div className="flex justify-start">
            <p className="font-mono text-cyan-400 text-xs animate-pulse">
              {isThinking ? 'â— Denke nachâ€¦' : 'â—† Sprecheâ€¦'}
            </p>
          </div>
        )}
        ...
        {micError && (
          <div className="flex justify-start">
            <p className="font-mono text-red-400/80 text-xs">âš  {micError}</p>
          </div>
        )}
      ...
      <div className="w-full max-w-xl px-6 flex items-center gap-2">
        <input
          ...
          onKeyDown={handleKeyDown}
          disabled={isThinking || isSpeaking}
          placeholder="Frage Jarvis etwasâ€¦"
          className="flex-1 bg-gray-900/80 border border-cyan-400/30 text-gray-200 placeholder-gray-600 font-mono text-sm rounded px-4 py-2 outline-none focus:border-cyan-400/60 transition-colors disabled:opacity-40"
        />

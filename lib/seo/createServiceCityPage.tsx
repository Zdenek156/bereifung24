...
export function createServiceCityPage(serviceSlug: string) {
  ...
  async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return {
      ...
      alternates: {
        canonical: `https://bereifung24.de/${service.slug}/${city.citySlug}`,
      },
      openGraph: {
        title,
        description,
        type: 'website',
        url: `https://bereifung24.de/${service.slug}/${city.citySlug}`,
        siteName: 'Bereifung24',
        locale: 'de_DE',
        images: [{ url: 'https://bereifung24.de/og-image.jpg', width: 1200, height: 630, alt: `${service.name} in ${city.cityName}` }],
      },
      twitter: {
    ...
    const jsonLd = [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: service.name, item: `https://bereifung24.de/services/${service.slug}` },
          { '@type': 'ListItem', position: 3, name: city.cityName, item: `https://bereifung24.de/${service.slug}/${city.citySlug}` },
        ],
      },
    ]

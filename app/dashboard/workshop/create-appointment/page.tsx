...
export default function CreateAppointmentPage() {
  ...
  useEffect(() => {
    if (!selectedServiceId || selectedServiceId === 'manual') {
      return
    }
    // Check if it's a service:package combo
    if (selectedServiceId.includes(':')) {
      const [serviceId, packageId] = selectedServiceId.split(':')
      const service = services.find(s => s.id === serviceId)
      const pkg = service?.servicePackages?.find(p => p.id === packageId)
      if (pkg) {
        setPrice(pkg.price.toFixed(2))
      }
    } else {
      const service = services.find(s => s.id === selectedServiceId)
      if (service) {
        setPrice(service.basePrice.toFixed(2))
      }
    }
  ...
  const loadServices = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.workshopId) setWorkshopId(data.workshopId)
      }
    } catch (err) {
  ...
  const loadAvailableSlots = async () => {
    ...
    try {
      const response = await fetch(`/api/workshop/available-slots?date=${selectedDate}&workshopId=${workshopId}`)
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Termin erstellen</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400">Tragen Sie manuell einen neuen Termin ein</p>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-5">
          ...
          {customer && (
            <div className="mb-4 px-3 py-2.5 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Datum & Uhrzeit</h2>
                ...
                <div>
                  ...
                  {loadingSlots ? (
                    <div className="text-center py-3">
                      ...
                      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Lade verfügbare Zeiten...</p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="md:col-span-2">
                <label htmlFor="service" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                  Service *
                </label>
              {loadingServices ? (
              ...
              {/* Price - inline next to service */}
              <div>
                <label htmlFor="price" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                  Preis
                </label>
                <div className="relative">
                  <input
                    ...
                    step="0.01"
                    min="0"
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    placeholder="0.00"
                  />
            ...
            {selectedServiceId === 'manual' && (
              <div>
                <label htmlFor="customDuration" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                  Dauer in Minuten *
                </label>
                <input
                  type="number"
                  ...
                  required
                />
                <p className="mt-0.5 text-xs text-gray-400">Vielfache von 15 Minuten empfohlen</p>
              </div>
            )}
            ...
            {customer && customer.vehicles && customer.vehicles.length > 0 && (
              <div>
                <label htmlFor="vehicle" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                  Fahrzeug
                </label>
                <select
                  id="vehicle"
            ...
            {/* Remove old Price block - now inline above */}

            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider mb-3">
                Zusätzliche Informationen <span className="text-xs font-normal text-gray-400 normal-case">(Optional)</span>
              </h2>

              {/* Kunden-Informationen (nur wenn kein Customer geladen) */}
              {!customer && (
                <div className="space-y-3 mb-4">
                  <h3 className="text-xs font-medium text-gray-500 dark:text-gray-400">Kundeninformationen</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      <label htmlFor="customerName" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Name</label>
                      <input
                        type="text"
                        id="customerName"
                        value={customerName}
                        onChange={(e) => setCustomerName(e.target.value)}
                        className="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                        placeholder="Max Mustermann"
                      />
                    </div>
                    <div>
                      <label htmlFor="customerPhone" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Telefon</label>
                      <input
                        type="tel"
                        id="customerPhone"
                        value={customerPhone}
                    ...
                    <div>
                      <label htmlFor="customerEmail" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">E-Mail</label>
                      <input
                        type="email"
                        id="customerEmail"
                        value={customerEmail}
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-gray-500 dark:text-gray-400">Service & Fahrzeug</h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="serviceDescription" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Service-Beschreibung</label>
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                      placeholder="z.B. Reifenwechsel, Ölwechsel..."
                    />
                  </div>

                  {/* Fahrzeug-Info (nur wenn kein Fahrzeug ausgewählt) */}
                  {!selectedVehicleId && (
                    <div>
                      <label htmlFor="vehicleInfo" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Fahrzeug</label>
                      <input
                        type="text"
                        id="vehicleInfo"
                        value={vehicleInfo}
                        ...
                        className="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                        placeholder="z.B. VW Golf, B-AB 1234"
                      />
                    </div>
                  )}
                </div>
              ...
              {/* Notizen */}
              <div className="mt-3">
                <label htmlFor="notes" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Interne Notizen</label>
                <textarea
                  id="notes"
                  value={notes}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
              <Link
                ...
                className="px-4 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
              >
                Abbrechen
              </Link>
              <button
                ...
                className="px-5 py-1.5 bg-primary-600 text-white rounded-lg text-sm hover:bg-primary-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
              >
                {submitting ? (
                  <>
        ...
        {/* Info Hinweis */}
        <div className="mt-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg px-4 py-3 flex items-start gap-2.5">
          ...
          <div className="text-xs text-green-800 dark:text-green-300">
            <strong>Kostenloser Service von Bereifung24:</strong> Die manuelle Terminverwaltung und die hier eingetragenen Services haben nichts mit der Provision zu tun. Dieser Kalender ist ein kostenloses Werkzeug für Ihre Werkstatt-Organisation — Sie können hier alle Termine eintragen, unabhängig davon ob sie über Bereifung24 gebucht wurden.
          </div>
        </div>
      </main>
    </div>

    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <>
          {/* Stripe Dispute Webhook Secret */}
          <div className="bg-white rounded-lg shadow-lg mt-8">
            <div className="p-6 border-b border-gray-200">
              <label className="block">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-900">
                    STRIPE_WEBHOOK_SECRET_DISPUTES
                  </span>
                </div>
                <p className="text-xs text-gray-500 mb-2">
                  Webhook Secret für Stripe Dispute/Snapshot-Endpoint (zweiter Webhook für Chargebacks). Wird für charge.dispute.created etc. benötigt.
                </p>
                <div className="relative">
                  <input
                    type={visibleKeys['STRIPE_WEBHOOK_SECRET_DISPUTES'] ? "text" : "password"}
                    value={editedValues['STRIPE_WEBHOOK_SECRET_DISPUTES'] || ''}
                    onChange={e => handleValueChange('STRIPE_WEBHOOK_SECRET_DISPUTES', e.target.value)}
                    placeholder="whsec_..."
                  ...
                  <button
                    type="button"
                    onClick={() => toggleVisibility('STRIPE_WEBHOOK_SECRET_DISPUTES')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 focus:outline-none"
                  >
                    {visibleKeys['STRIPE_WEBHOOK_SECRET_DISPUTES'] ? (

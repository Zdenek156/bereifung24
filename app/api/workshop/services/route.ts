...
export async function GET(request: Request) {
  try {
    ...
    const services = user.workshop.workshopServices.map(service => ({
      ...
      washingPrice: service.washingPrice,
      washingAvailable: service.washingAvailable,
      ...
      acceptsMountingOnly: service.acceptsMountingOnly,
      mountingOnlySurcharge: service.mountingOnlySurcharge,
  ...
  try {
    ...
    const service = await prisma.workshopService.create({
      data: {
        ...
        washingPrice: (serviceType === 'WHEEL_CHANGE' && washingPrice) ? parseFloat(washingPrice) : null,
        refrigerantPricePer100ml: (serviceType === 'CLIMATE_SERVICE' && refrigerantPrice) ? parseFloat(refrigerantPrice) : null,
        acceptsMountingOnly: acceptsMountingOnly !== undefined ? acceptsMountingOnly : true,
        mountingOnlySurcharge: mountingOnlySurcharge !== undefined ? parseFloat(mountingOnlySurcharge) : 0,
        ...
        washingAvailable: (serviceType === 'WHEEL_CHANGE' && washingAvailable) ? washingAvailable : false,
        allowsDirectBooking: (serviceType === 'WHEEL_CHANGE' && allowsDirectBooking) ? allowsDirectBooking : false,

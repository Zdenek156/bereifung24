...
export function useJarvis() {
  ...
  // Keyboard sequence listener â€” type "jarvis" to open
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loadGreeting = async () => {
      ...
      } catch { /* fall through with no stats */ }

      try {
        const res = await fetch('/api/jarvis/greeting', {
          method: 'POST',
        ...
        setMessages([{ role: 'jarvis', text: 'Jarvis online. Wie kann ich Ihnen behilflich sein, Sir?' }])
      }
    }
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'jarvis', text: 'Verzeihen Sie, Sir. Es gab eine kurze SystemstÃ¶rung.' }])
      setIsThinking(false)
    }
